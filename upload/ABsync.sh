...
echo
if [ "$4" = "1" ];then
    rclone move "openlist:/$A" "openlist:/$B" -P --transfers=16 --size-only --config ./openlist/rclone.conf
else
    rclone copy "openlist:/$A" "openlist:/$B" -P --transfers=16 --size-only --config ./openlist/rclone.conf
fi
echo
