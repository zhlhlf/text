...
def backup_data(host, username, password):
    ...
    try:
        ...
        with open('openlist_backup.json', 'w', encoding='utf-8') as f:
            json.dump(backup, f, ensure_ascii=False, indent=4)
        
        print("备份完成：openlist_backup.json")
        return True
    except Exception as e:
        print(f"备份失败: {str(e)}")
        return False
...
def restore_data(host, username, password):
    ...
    try:
        ...
        print("开始恢复数据...")
        with open('openlist_backup.json', 'r', encoding='utf-8') as f:
            data = json.load(f)
